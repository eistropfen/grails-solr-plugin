...
class SolrUtil {
  ...
  */
  ...
  def static final typeSuffixes = ["_i", "_s", "_l", "_t", "_b", "_f", "_d", "_dt", "_ti", "_tl", "_tf", "_td", "_pi"]
  ...
  */
  ...
  static SolrInputDocument addFieldToDoc(SolrInputDocument doc, FieldSolrMeta fieldMeta, Object value, String prefix) {
    def docKey = "${prefix}${fieldMeta.fullSolrFieldName}"
    def docValue = value

    // then set the value to the Solr Id
    if (docValue && DomainClassArtefactHandler.isDomainClass(docValue.class)) {
      docValue = SolrUtil.getSolrId(docValue)
    }

    doc.addField(docKey, docValue)

    if (fieldMeta.asTextAlso) {
      doc.addField("${prefix}${fieldMeta.solrFieldPrefix}${fieldMeta.propertyName}_t", docValue)
    }

    doc
  }

  static void addTypeToDoc(SolrInputDocument doc, Class clazz, String prefix) {
    def docKey = prefix + SolrUtil.TYPE_FIELD
    def docValue = clazz.name

    doc.addField(docKey, docValue)
  }

  static void addIdToDoc(SolrInputDocument doc, Class clazz, Long id, String prefix) {
    def docKey = "${prefix}id"
    def docValue = "${clazz.name}-${id}"
